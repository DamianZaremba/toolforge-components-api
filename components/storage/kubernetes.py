...
from kubernetes_asyncio import client as k8s_client  # type: ignore
...
class KubernetesStorage(Storage):
    ...
    async def get_tool_config(self, tool_name: str) -> ToolConfig:
        ...
        try:
            k8s_tool_config = await self.k8s_client.get_namespaced_custom_object(
                group="toolforge.org",
                version="v1",
                name=config_name,
                plural="toolconfigs",
    ...
    async def set_tool_config(self, tool_name: str, config: ToolConfig) -> None:
        try:
            await self._create_tool_config(tool_name=tool_name, config=config)
        except k8s_client.ApiException as error:
            if error.status == status.HTTP_409_CONFLICT:
                # it already exists, just update
                await self._delete_tool_config(tool_name=tool_name)
                await self._create_tool_config(tool_name=tool_name, config=config)
            else:
                raise Exception(
    ...
    async def delete_tool_config(self, tool_name: str) -> ToolConfig:
        old_tool_config = await self.get_tool_config(tool_name=tool_name)
        await self._delete_tool_config(tool_name=tool_name)
        return old_tool_config

    async def _create_tool_config(self, tool_name: str, config: ToolConfig) -> None:
        namespace = _get_k8s_tool_namespace(tool_name=tool_name)
    ...
    async def _delete_tool_config(self, tool_name: str) -> None:
        namespace = _get_k8s_tool_namespace(tool_name=tool_name)
        config_name = _get_k8s_tool_config_name(tool_name)
        try:
    ...
    async def get_deployment(self, tool_name: str, deployment_name: str) -> Deployment:
        namespace = _get_k8s_tool_namespace(tool_name=tool_name)
        try:
            k8s_deployment = await self.k8s_client.get_namespaced_custom_object(
                group="toolforge.org",
                version="v1",
    ...
    async def get_deploy_token(self, tool_name: str) -> DeployToken:
        namespace = _get_k8s_tool_namespace(tool_name=tool_name)
        try:
            k8s_token = await self.k8s_client.get_namespaced_custom_object(
                group="toolforge.org",
                version="v1",
                name=tool_name,
                plural="deploytokens",
    ...
    async def _set_deploy_token_envvar(
        self, tool_name: str, token: DeployToken
    ) -> None:
        ...
        try:
            response_data = await self.toolforge_client.post(
    ...
    async def _set_deploy_token_crd(self, tool_name: str, token: DeployToken) -> None:
        namespace = _get_k8s_tool_namespace(tool_name=tool_name)
    ...
    async def set_deploy_token(self, tool_name: str, token: DeployToken) -> None:
        try:
            await self._set_deploy_token_crd(tool_name=tool_name, token=token)
        except k8s_client.ApiException as error:
            if error.status == status.HTTP_409_CONFLICT:
                await self._delete_deploy_token_crd(tool_name=tool_name)
                await self._set_deploy_token_crd(tool_name=tool_name, token=token)
            else:
                raise Exception(
                    "Unexpected unhandled k8s ApiException, should not have reached here."
        ...
        await self._set_deploy_token_envvar(tool_name=tool_name, token=token)

    async def _delete_deploy_token_envvar(self, tool_name: str) -> None:
        settings = get_settings()

        try:
            await self.toolforge_client.delete(
    ...
    async def _delete_deploy_token_crd(self, tool_name: str) -> None:
        namespace = _get_k8s_tool_namespace(tool_name=tool_name)
    ...
    async def delete_deploy_token(self, tool_name: str) -> DeployToken:
        token = await self.get_deploy_token(tool_name)

        await self._delete_deploy_token_crd(tool_name=tool_name)
        await self._delete_deploy_token_envvar(tool_name=tool_name)
        return token
